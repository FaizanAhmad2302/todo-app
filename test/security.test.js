...
describe("Security Requirements", () => {
  test("Isolation: User A cannot see/edit/delete User B's Todo", async () => {
    ...
    const cookiesA = await loginUser(
      baseUrl,
      "usera@example.com",
      "password123"
    );
    const cookiesB = await loginUser(
      baseUrl,
      "userb@example.com",
      "password123"
    );
  ...
  test("Reuse: Reusing an old refresh token revokes all sessions for that user", async () => {
    ...
    const cookies1 = await loginUser(
      baseUrl,
      "reuse@example.com",
      "password123"
    );
    ...
    const test1 = await fetch(`${baseUrl}/todos`, {
    ...
    const test2 = await fetch(`${baseUrl}/todos`, {
      headers: { Cookie: cookies2 },
    });
