...
describe("Regression Tests", () => {
  ...
  test("Test 3: Creating a Todo with a mismatched token returns 401", async () => {
    ...
    const invalidCookies = cookies.replace(
      /accessToken=[^;]+;/,
      "accessToken=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjEiLCJyb2xlIjoidXNlciIsImlhdCI6MTYzMjQ4NzQyMCwiZXhwIjoxNjMyNDg4MzIwfQ.x;"
    );
  ...
  test("Test 4: Creating a Todo without CSRF Origin/Referer header fails (outside test environment)", async () => {
    ...
    try {
      ...
      assert.strictEqual(
        response.status,
        403,
        "Expected 403 Forbidden due to missing Origin/Referer"
      );
    } finally {
  ...
  test("Test 5: Refresh token can successfully generate a new access token", async () => {
    ...
    assert.ok(
      newCookies.includes("accessToken="),
      "Expected new accessToken in set-cookie"
    );
    assert.ok(
      newCookies.includes("refreshToken="),
      "Expected new refreshToken in set-cookie"
    );
  });
  ...
  test("Test 6: A disabled user cannot create a Todo even with a valid token", async () => {
    ...
    assert.strictEqual(
      response.status,
      401,
      "Expected 401 Unauthorized for disabled user"
    );
  });
});
